...
-- note that the cases here cannot be separated into their own line, because 
-- that would lead to ambiguous syntax, trying to parse the cases as a new statement.
test( 10,
  match {2, {function(x) return x * 2 end, 5}, 'hi'} ( case{2, var'x'} - 'false', case{2, {var'doub', 5}, 'hi'} - ' doub(5) ' )
)

test( 'number',
  match_all {1, 3, a = 5, b = 7} ( case{var'_', var'_', a = var'_', b = var'x'} - DO('type(x)', {}) )
)
